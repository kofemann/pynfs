...
def testCreateUncheckedFile(t, env):
    ...
    res = c.create_file(t.word(), attrs=orig_attrs,  deny=OPEN4_SHARE_DENY_NONE)
    check(res, msg="Trying to create file %s" % t.word())
    ...
    res = c.create_file(t.word(), attrs=attrs,  deny=OPEN4_SHARE_DENY_NONE)
    check(res, msg="Trying to recreate file %s" % t.word())
    fh, stateid = c.confirm(t.word(), res)
    rcvd_attrs = c.do_getattrdict(fh, orig_attrs.keys())
    checkdict(orig_attrs, rcvd_attrs, get_bitnumattr_dict(),
    ...
    res = c.create_file(t.word(), attrs=attrs,  deny=OPEN4_SHARE_DENY_NONE)
    check(res, msg="Trying to truncate file %s" % t.word())
    fh, stateid = c.confirm(t.word(), res)
    rcvd_attrs = c.do_getattrdict(fh, orig_attrs.keys())
...
def testCreatGuardedFile(t, env):
    ...
    c = env.c1
    c.init_connection()
    res = c.create_file(t.word(), mode=GUARDED4)
    check(res, msg="Trying to do guarded create of file %s" % t.word())
    c.confirm(t.word(), res)
    # Create the file again, should return an error
    res = c.create_file(t.word(), mode=GUARDED4)
    check(res, NFS4ERR_EXIST,
          "Trying to do guarded recreate of file %s" % t.word())
...
def testOpenFile(t, env):
    ...
    c = env.c1
    c.init_connection()
    c.open_confirm(t.word(), env.opts.usefile)
...
def testDir(t, env):
    ...
    c = env.c1
    c.init_connection()
    res = c.open_file(t.word(), env.opts.usedir)
...
def testLink(t, env):
    ...
    c = env.c1
    c.init_connection()
    res = c.open_file(t.word(), env.opts.uselink)
...
def testBlock(t, env):
    ...
    c = env.c1
    c.init_connection()
    res = c.open_file(t.word(), env.opts.useblock)
...
def testChar(t, env):
    ...
    c = env.c1
    c.init_connection()
    res = c.open_file(t.word(), env.opts.usechar)
...
def testSocket(t, env):
    ...
    c = env.c1
    c.init_connection()
    res = c.open_file(t.word(), env.opts.usesocket)
...
def testFifo(t, env):
    ...
    c = env.c1
    c.init_connection()
    res = c.open_file(t.word(), env.opts.usefifo)
...
def testLongName(t, env):
    ...
    c = env.c1
    c.init_connection()
    res = c.create_file(t.word(),  c.homedir + [env.longname])
...
def testInvalidUtf8(t, env):
    ...
    for name in get_invalid_utf8strings():
        res = c.create_file(t.word(), c.homedir + [t.word(), name])
...
def testInvalidAttrmask(t, env):
    ...
    c = env.c1
    c.init_connection()
    res = c.create_file(t.word(), attrs={FATTR4_LINK_SUPPORT: TRUE})
...
def testUnsupportedAttributes(t, env):
    ...
    for attr in env.attr_info:
        if attr.writable and not supported & attr.mask:
            count += 1
            res = c.create_file(t.word(), attrs={attr.bitnum : attr.sample})
...
def testClaimPrev(t, env):
    ...
    res = c.open_file(t.word(), fh, claim_type=CLAIM_PREVIOUS, deleg_type=OPEN_DELEGATE_NONE)
...
def testModeChange(t, env):
    ...
    res = c.compound(ops)
    check(res, msg="Setting mode of file %s to 0o000" % t.word())
    res = c.open_file(t.word(), access=OPEN4_SHARE_ACCESS_BOTH,
                      deny=OPEN4_SHARE_DENY_NONE)
    if env.opts.uid == 0:
        check(res, [NFS4_OK, NFS4ERR_ACCESS], "Opening file %s with mode=0o000" % t.word())
    else:
        check(res, NFS4ERR_ACCESS, "Opening file %s with mode=0o000" % t.word())
...
def testShareConflict1(t, env):
    ...
    c.create_confirm(t.word(), file, access=OPEN4_SHARE_ACCESS_BOTH)
    res = c.open_file(t.word(2), file, deny=OPEN4_SHARE_DENY_WRITE)
...
def testDenyRead2(t, env):
    ...
    c.init_connection()
    file = c.homedir + [t.word()]
    fh, stateid = c.create_confirm(t.word(1), file,
                                   access=OPEN4_SHARE_ACCESS_BOTH,
                                   deny=OPEN4_SHARE_DENY_READ)
    res = c.open_file(t.word(2), file, access=OPEN4_SHARE_ACCESS_READ,
                      deny=OPEN4_SHARE_DENY_NONE)
    check(res, NFS4ERR_SHARE_DENIED,
          "OPEN with access==read on a read-denied file")
...
def testDenyWrite1(t, env):
    ...
    # Same owner, should fail despite already having read access
    # This is stated in both 14.2.16 and 8.9
    res = c.open_file(t.word(), access=OPEN4_SHARE_ACCESS_WRITE,
                      deny=OPEN4_SHARE_DENY_NONE)
    check(res, NFS4ERR_SHARE_DENIED,
          "OPEN with access==write on a write-denied file")
...
def testDenyWrite2(t, env):
    ...
    fh, stateid = c.create_confirm(t.word(1), file,
                                   access=OPEN4_SHARE_ACCESS_BOTH,
                                   deny=OPEN4_SHARE_DENY_WRITE)
    res = c.open_file(t.word(2), file, access=OPEN4_SHARE_ACCESS_WRITE,
                      deny=OPEN4_SHARE_DENY_NONE)
    check(res, NFS4ERR_SHARE_DENIED,
          "OPEN with access==write on a write-denied file")
